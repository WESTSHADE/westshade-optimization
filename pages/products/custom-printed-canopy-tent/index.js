...
import {
    Backdrop, Box, Breadcrumbs, Container, CircularProgress, Divider, FormControlLabel, FormControl, Grid, List,
    ListItem, ListItemText, Radio, RadioGroup, Tabs, Tab, Typography,
    ...
    return (
        <Radio
            ...
            checkedIcon={
                <>
                    {props.attribute === "wall-type" ? (
                        <div className="section-radio-outer-border"/>
                    ) : props.attribute === "canopy-color" ? (
                        <div className="section-radio-border-circle"/>
                    ) : null}
                    {icon}
                </>
            }
...
function TabPanelRoof(props) {
    ...
    const {index, value, handleChangeRadio, selectedAttribute, ...other} =
        props;
            ...
            {value === index && (
                <>
                    <div className="section-selection-container">
                        ...
                        <Tabs
                            value={tabPeakColorType}
                            onChange={handleChangeTabPeakColorType}
                                ...
                                >
                                    {SelectionColor.map((option, index) => (
                                        <FormControlLabel
                                            ...
                                            control={<StyledRadio attribute={"canopy-color"}/>}
                                        />
                                    ))}
                                </RadioGroup>
                            </FormControl>
                    ...
                    <div className="section-selection-container">
                        ...
                        <Tabs
                            value={tabValanceColorType}
                            onChange={handleChangeTabValanceColorType}
                            TabIndicatorProps={{hidden: true}}
                        >
                            <Tab
                                className="text-tab"
                        ...
                        {!tabValanceColorType ? (
                            <FormControl component="fieldset">
                                <RadioGroup
                                    row
                                    defaultValue={"white"}
                                    value={valueValanceType}
                                    onChange={(event) =>
                                        handleChangeRadio(
                                            event,
                                            (index + 1) * 2 + 1,
                                            id_attribute_valanceType
                                        )
                                    }
                                >
                                    {SelectionColor.map((option, index) => (
        ...
        <div key={index} role="tabpanel" hidden={value !== index} {...other}>
            {value === index && (
                <>
                    <div className="section-selection-container">
                        <Tabs
                            value={tabWallColorType}
                            onChange={handleChangeTabWallColorType}
                            TabIndicatorProps={{hidden: true}}
                        >
                            <Tab
                                className="text-tab"
                                    ...
                                    >
                                        {SelectionWallType.map((option, index) => (
                                            <FormControlLabel
                                                ...
                                                className={classes.root}
                                                value={option.toLowerCase()}
                                                control={<StyledRadio attribute={"wall-type"}/>}
                                                label={option}
                                ...
                                .map((attribute, index) => {
                                    ...
                                    return (
                                        <div key={index} className="section-selection-container">
                                            <Selections
                                                defaultValue={stringFn.replaceDash(
                                                    productComponent[14 + index].default_attributes[index]
                                                        .option,
                                                    1
                                                )}
                                                onChange={(event) =>
                                                    handleChangeRadio(event, 14 + index, attribute.id)
                                                }
...
export default function Canopy_Tent() {
    ...
    const fetchCheckOut = async (string) => {
        if (!string) return;

        return await utils.checkoutWoo(string);
    };

    const fetchCheckOutRecursive = (list, limit = Number.MAX_VALUE) =>
        fetchCheckOut(list[list.length - limit]).then((res) => {
            if (limit > -1 && --limit) {
                return fetchCheckOutRecursive(list, limit);
            }
            return res;
        });
    ...
    const handleChangeRadio = (event, index, id) => {
        ...
        if (id === 4) {
            ...
            selection[10].map((attribute) => {
                if (attribute.id === 14)
                    attribute.option = sizes[0] === "26" ? "13ft" : sizes[0] + "ft";
            });
        ...
        if (id === id_attribute_canopySize) {
            let selectedA = productVariant[1].filter((variant) => {
                ...
                for (let i = 0; i < variant.attributes.length; i++) {
                    if (
                        variant.attributes[i].option.toLowerCase() !==
                        selection[1][i].option.toLowerCase()
                    ) {
                        equal = false;
                        break;
                    }
            ...
            let selectedB = productVariant[2].filter((variant) => {
                ...
                for (let i = 0; i < variant.attributes.length; i++) {
                    if (
                        variant.attributes[i].option.toLowerCase() !==
                        selection[2][i].option.toLowerCase()
                    ) {
                        equal = false;
                        break;
                    }
            ...
            let selectedC = productVariant[3].filter((variant) => {
                ...
                for (let i = 0; i < variant.attributes.length; i++) {
                    if (
                        variant.attributes[i].option.toLowerCase() !==
                        selection[3][i].option.toLowerCase()
                    ) {
                        equal = false;
                        break;
                    }
            ...
            let selectedD = productVariant[4].filter((variant) => {
                ...
                for (let i = 0; i < variant.attributes.length; i++) {
                    if (
                        variant.attributes[i].option.toLowerCase() !==
                        selection[4][i].option.toLowerCase()
                    ) {
                        equal = false;
                        break;
                    }
    ...
    const handleMainImage = () => {
        ...
        let color = selectedVariant[1]
            ? selectedVariant[1].attributes
                .filter((attribute) => attribute.id === id_attribute_roofColor)[0]
                .option.toLowerCase()
            : "white";
        let c =
            color === "white"
        ...
        let size = selectedVariant[0].attributes
            .filter((attribute) => attribute.id === id_attribute_canopySize)[0]
            .option.toUpperCase();
    ...
    useEffect(() => {
        ...
        // 获取保存各组件信息
        Promise.all(
            product.composite_components.map((conponent) =>
                fetchProduct(conponent.default_option_id)
            )
        ).then((result) => setProductComponent(result));
    }, [product]);
    ...
    useEffect(() => {
        ...
        Promise.all(
            [
                {id: productComponent[0].id},
                {id: productComponent[1].id},
                {id: productComponent[2].id},
                {id: productComponent[3].id},
                {id: productComponent[10].id},
                {id: productComponent[14].id},
            ].map((component) => fetchProductVariant(component.id))
        ).then((result) =>
            setProductVariant([
                result[0],
                result[1],
    ...
    useEffect(() => {
        ...
        selectedAttribute.map((attribute, index) => {
            ...
            let selected = productVariant[index].filter((variant) => {
                ...
                if (index > 9 && index < 14) {
                    if (wallStatus[index - 10] === 0 || wallStatus[index - 10] === 2)
                        return false;
                }

                if (index > 13 && index < 18) {
                    if (wallStatus[index - 14] === 0 || wallStatus[index - 14] === 1)
                        return false;
                }
                ...
                for (let i = 0; i < variant.attributes.length; i++) {
                    if (
                        variant.attributes[i].option.toLowerCase() !==
                        attribute[i].option.toLowerCase()
                    ) {
                        equal = false;
                        break;
                    }
            ...
            if (item === 0) {
                ...
                let selected = productVariant[index + 10].filter((variant) => {
                    ...
                    for (let i = 0; i < variant.attributes.length; i++) {
                        if (
                            variant.attributes[i].option.toLowerCase() !==
                            selectedAttribute[index + 10][i].option.toLowerCase()
                        ) {
                            equal = false;
                            break;
                        }
                ...
                let selected = productVariant[index + 14].filter((variant) => {
                    ...
                    for (let i = 0; i < variant.attributes.length; i++) {
                        if (
                            variant.attributes[i].option.toLowerCase() !==
                            selectedAttribute[index + 14][i].option.toLowerCase()
                        ) {
                            equal = false;
                            break;
                        }
    ...
    useEffect(() => {
        ...
        selectedVariant.map((variant, index) => {
            ...
            if (index > 9 && index < 14) {
                // Set墙面图片
                let type = variant.attributes
                    .filter((v) => v.id === 11)[0]
                    .option.toLowerCase();
                let size = selectedVariant[0].attributes
                    .filter((v) => v.id === 4)[0]
                    .option.toLowerCase();
                let color = variant.attributes
                    .filter((v) => v.id === 3)[0]
                    .option.toLowerCase();
                wallPicturesList[index - 1] =
                    ...
                    "-" +
                    setWallImageUrl(index - 9, type, size, color);
            }

            regularPrice += variant.regular_price
                ? parseFloat(variant.regular_price) * totalCount
                : 0;
            salePrice += variant.on_sale
                ? variant.sale_price
                    ? parseFloat(variant.sale_price) * totalCount
                    : 0
        ...
        if (
            ...
            selectedVariant[0].attributes[1].option === "13x26"
        ) {
            if (selectedVariant[2]) {
                regularPrice += selectedVariant[2].regular_price
            ...
            if (selectedVariant[4]) {
                regularPrice += selectedVariant[4].regular_price
    ...
    useEffect(() => {
        ...
        availableList.map((a, i) => {
            if (a.status) {
                ...
                if (i === 2 || i === 4) {
                    let aa = selectedAttribute[i].find(
                        (attribute) => attribute.id === 14
                    );
                    if (aa && aa.option === "13ft") {
                        if (index > -1) {
                            selectedProduct[index].needed += a.needed * 2;
                            if (
                                selectedProduct[index].needed > selectedProduct[index].quantity
                            ) {
                                av = false;
                            }
                        } else {
                ...
                if (i > 0) {
                    let bb = selectedAttribute[i].find(
    ...
    const checkOut = () => {
        ...
        fetchCheckOutRecursive(productList, productList.length)
            .then((data) => window.open("https://www.westshade.com/cart", "_self"))
            .catch((error) => {
            });
    };
                        ...
                        <Grid item xs={12} sm={6}>
                            <div className={styles["container-product-image"]}>
                                ...
                                {wallPictures.map((picture, index) => {
                                    if (!picture) return;
                                    return (
                                        <img key={index} className={styles["product-image"]} src={pic}/>
                                    );
                                })}
                            </div>
                        </Grid>
                                            ...
                                            <div key={index} className="section-selection-container">
                                                <Selections
                                                    label={
                                                        <div>
                                                            {"Choose your " +
                                                            stringFn.replaceDash(attribute.name, 1)}
                                                            <div
                                                                style={{
                                                                    marginTop: 6,
                                                                    marginBottom: 18,
                                                            ...
                                                            >
                                                                {attribute.id === id_attribute_frameSeries
                                                                    ? "Which series is right for you?"
                                                                    : attribute.id === id_attribute_canopySize
                                                    ...
                                                    defaultValue={
                                                        productComponent[0].default_attributes[index].option
                                                    }
                                                    onChange={(event) =>
                                                        handleChangeRadio(event, 0, attribute.id)
                                                    }
                                                    list={
                                                        attribute.id === id_attribute_canopySize
                            ...
                            <div>
                                {productComponent && productComponent[1] ? (
                                    <div className="section-selection-container">
                                        ...
                                        <Tabs
                                            value={tabRoofColorType}
                                            onChange={handleChangeTabRoofColorType}
                                            TabIndicatorProps={{hidden: true}}
                                        >
                                            <Tab
                                                className="text-tab"
                                            ...
                                            productComponent[1].attributes
                                                ...
                                                .map((attribute, index) => {
                                                    ...
                                                    return (
                                                        <div
                                                            key={index}
                                                            className="section-selection-container"
                                                        >
                                                            <Selections
                                                                defaultValue={
                                                                    productComponent[1].default_attributes[index]
                                                                        .option
                                                                }
                                                                onChange={(event) =>
                                                                    handleChangeRadio(event, 1, attribute.id)
                                                                }
                                                                list={attribute.options}
                                                                id={attribute.id.toString()}
                                                            />
                        ...
                        {productId === "25649" ? (
                            ...
                            <Grid container spacing={6}>
                                <Grid item xs={12} md={6}>
                                    <img
                                        src="/images/y5-display.png"
                                        style={{objectFit: "contain", width: "100%", height: 320}}
                                    />
                                </Grid>
                                <Grid
                                    item
                                ...
                                >
                                    <div>
                                        ...
                                        <p className="section-desc-container-content">
                                            Our Y5 economic canopy series is the best outdoor patio
                                            canopy for daily use.
                                            <br/>
                                    ...
                                    <ul className="section-desc-container-content">
                                        ...
                                        <li>
                                            Steel made frame, zinc coated nuts and bolts, nylon
                                            bracket connectors.
                                        </li>
                                        <li>
                                        ...
                                        <li>
                                            Instant pop up with one release button for 3 different
                                            heights adjustment from 5’2″ to 6’8″.
                                        </li>
                                        <li>
                            ...
                            <Grid container spacing={6}>
                                ...
                                >
                                    <div>
                                        ...
                                        <p className="section-desc-container-content">
                                            Our Y6 commercial canopy series is the ultimate solution
                                            for small business and multi families. Stronger but
                                            lighter.
                                            <br/>
                                            Available in 3 sizes and 6 colors.
                                        </p>
                            ...
                            <Grid container spacing={6}>
                                ...
                                >
                                    <div>
                                        ...
                                        <p className="section-desc-container-content">
                                            Our Y7 heavy duty canopy series is the best heavy-duty
                                            canopy on the market. Unbeatable strength and durability
                                            while staying lightweight. Ideal option for commercial
                                            use.
                                            <br/>
                        ...
                        >
                            <List>
                                <ListItem className="background-light-blue">
                                    ...
                                    {productId === "25649" || productId === "24229" ? (
                                        <ListItemText
                                            ...
                                            primary={"100 sq. ft, 150 sq. ft, 200 sq. ft"}
                                        />
                                    ) : null}
                                    {productId === "25659" ? (
                                        <ListItemText
                                            className="section-image-package-listItem-content section-image-package-listItem-subtitle-wrap"
                                            primary={
                                                "100 sq. ft, 150 sq. ft, 200 sq. ft, 169 sq. ft, 256 sq. ft, 260 sq. ft, 338 sq. ft, 400 sq. ft"
                                            }
                                        />
                                    ) : null}
                                </ListItem>
                                <ListItem>
                                    ...
                                    {productId === "24229" || productId === "25659" ? (
                                        <ListItemText
                                            className="section-image-package-listItem-content section-image-package-listItem-subtitle-wrap"
                                            primary={"Aluminum"}
                                        />
