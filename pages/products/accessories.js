...
function Accessories({router, product, productComponent, productVariant}) {
    ...
    useEffect(() => {
        ...
        if (uProduct.type === "simple" || uProduct.type === "variable") {
            ...
            Promise.all(
                uProduct.composite_components.map(({default_option_id}) => fetchProduct(default_option_id))
            ).then((responses) => setProductComponent(responses));
        }
    }, [uProduct]);
                ...
                uProductComponent.map((component) => {
                    ...
                    if (component.id === id_product_wall) {
                        defaultAttr.forEach((attr) =>
                            wallType && attr.id === id_attribute_wallType ? (attr.option = wallType) : null
                        );
                    }
    ...
    const checkOut = () => {
        fetchCheckOutRecursive(checkoutProductList, checkoutProductList.length)
                            ...
                            <Grid item xs={12} sm={6} className="position-r">
                                ...
                                <div>
                                    {uProductComponent && uProductComponent[0]
                                        ? uProductComponent[0].attributes.filter((attribute) => attribute.variation).map((attribute, index) => {
