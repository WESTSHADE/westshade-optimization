...
const Index = ({product, productVariant, productComponent}) => {
    const {loggedIn, token, user} = useSelector(({user}) => user);
    const reduxDispatch = useDispatch();
    const {cart} = useSelector(({cart}) => cart);
    const [state, dispatch] = useReducer(reducer, initialState);
    const [steps, stepDispatch] = useReducer(stepReducer, initialSteps);
    ...
    const handleSendDetails = async () => {
        const res = await utils.contact({
            ...
            17: (state.printReq.peak.LEFT.text.content ? "Peak: " + state.printReq.peak.LEFT.text.content + "; " : "") + (state.printReq.valance.LEFT.text.content ? "Valance: " + state.printReq.peak.LEFT.text.content + ";" : ""),
        ...
        <Block width="100%" position="relative">
            {
                !state.activeCustomizer ?
                    <Block maxWidth="1152px" margin="0 auto">
                        <Block width="100%" padding={["40px 32px"]} display="flex" justifyContent="space-between" flexWrap={["wrap", "wrap", "nowrap"]}>
                            <Block flex="1" marginRight={["70px"]} backgroundColor="#F2F2F2">
                                <Image src={state.finalImage} alt="product image" width={712} height={520} layout="responsive" />
                            </Block>
                            <Block maxWidth="370px">
                                <Block as="h1" font="MinXHeading20">
                                    Custom Printed Canopy Tent
                                </Block>
                                <Block color="MinXPrimaryText" as="h2" font="MinXHeading16">
                                    Y7 Heavy duty aluminum
                                </Block>
                                <Block marginTop="40px" color="MinXPrimaryText" className="price" font="MinXHeading16" display="flex">
                                ...
                                <Block marginTop="40px">
                                    ...
                                    <Block marginTop="12px">
                                        {
                                            !steps.done ?
                                            <ButtonM width="100%" type="rainbow" height={["36px", "48px", "56px"]} font={["MinXLabel14", "MinXLabel16", "MinXLabel20"]} color="MinXPrimaryText" text="Customize online" buttonBackgroundColor="#FAFBFF"
                                            ...
                                            >
                                                ...
                                                <Block marginTop="9px">
                                                        <Block display="flex">
                                                            ...
                                                            <Block font="MinXParagraph14" marginLeft="4px">
                                                                {state.size}
                                                            </Block>
                                                        </Block>
                                                        <Block display="flex">
                                    ...
                                    <Block font="MinXParagraph14" color="MinXTitle" marginTop="12px">
                                        <Block display="inline" color="#000000" font="MinXHeading14">All custom printing orders will get a mockup before production.</Block> You can also <Block display="inline" color="MinXButton" className={css({textDecoration: "underline"})}><Link href="//custom-printing">get a free mockup </Link></Block> without ordering.
                                    </Block>
                                    <Block marginTop="40px">
                                            <Block
                                                display="flex" 
                ...
                <>
                    ...
                    <Block margin="0 auto" maxWidth="1152px" width="100%" padding="38px 16px 90px">
                        {
                            steps.currentStep === 0 && <TentSizeSelection sizeValue={state.size} setSize={selectSize} frame={state.frame}/>
                        }
                        {
                            steps.currentStep === 1 && <FrameSelection frameValue={state.frame} setFrame={selectFrame} />
                        }
                        {
