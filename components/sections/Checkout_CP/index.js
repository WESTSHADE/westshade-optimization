...
const checkout = (props) => {
    const {totalPrice = 0, totalSalesPrice = 0, onSale = false, isAvailable = true, isInStock = true, buttonText = "Add to Bag", stepsDone = false} = props;

    return (
        <Block className={`${styles["container-checkout"]} main-container-checkout`} position="fixed" width="100vw" height={["94px", null, "64px"]} paddingRight={["16px", null, "32px"]} paddingLeft={["16px", null, "32px"]}>
            <Block display="flex" alignItems="center" maxWidth={process.env.maxWidth + "px"} height="100%" marginRight="auto" marginLeft="auto">
                {/* 屏宽 小于 480 */}
                {stepsDone ? (
                    <>
                        <Block position={"relative"} display={["block", null, "none"]} height="40px">
                        ...
                        <Block width={["100%", null, "448px", "100%"]} height={["54px", null, "100%"]} display={"flex"} flexDirection={"row"} justifyContent={["center", "center", "space-between"]} alignItems={"center"} margin={"auto"}>
                            <Block display={["none", null, null, "flex"]} alignItems="column" justifyContent="center">
                    ...
                    <MButton type="rainbow" width="100%" height="56px" font="MinXLabel14" color="MinXPrimaryText" text="Customize online" buttonBackgroundColor="#FFF" display={["none", null, "block"]}
                             onClick={props.onClickAddToBag}
                    />
                )}
            </Block>
