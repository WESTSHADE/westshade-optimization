...
import { AppBar, Badge, SwipeableDrawer, Button, Divider, IconButton, List, ListItem, ListItemIcon, ListItemText, Typography, Toolbar, Link, Collapse } from "@material-ui/core";
...
// 	return (
// 		<div onClick={handleDrawer(false)} onKeyDown={handleDrawer(false)}>
// 			<List key={index} style={{ position: "relative" }}>
// 				<ListItem button style={{ paddingTop: 2, paddingBottom: 2, paddingRight: 48 }} onClick={() => (list ? handleOpenList() : url ? handleRouterGo(url) : null)}>
// 					{url ? (
// 						<Link href={url}>
// 							<ListItemText primary={name} style={{ color: "white", letterSpacing: 0 }} />
// 						</Link>
// 					) : (
// 						<ListItemText primary={name} style={{ color: "white", letterSpacing: 0 }} />
// 					)}
...
// 				</ListItem>
// 				{list ? (
// 					<Collapse style={{ marginLeft: 24 }} in={openList} timeout="auto" unmountOnExit>
// 						{list.map((item, i) => (
...
export default function HeaderDrawerLeft() {
			...
			list: [
				{
					...
					list: [
						{ name: "10 x 10", url: "/custom-printed-package/f1010cpp" },
						{ name: "10 x 15", url: "/custom-printed-package/f1015cpp" },
						{ name: "10 x 20", url: "/custom-printed-package/f1020cpp" },
						{ name: "13 x 13", url: "/custom-printed-package/f1313cpp" },
						{ name: "13 x 20", url: "/custom-printed-package/f1320cpp" },
						{ name: "13 x 26", url: "/custom-printed-package/f1326cpp" },
						{ name: "16 x 16", url: "/custom-printed-package/f1616cpp" },
						{ name: "20 x 20", url: "/custom-printed-package/f2020cpp" },
					],
				},
				{
				...
				},
				{
					...
					list: [
						{ name: "Fitted Table Cover", url: "/custom-print/table-cover/fitted-table-cover" },
						{ name: "Stretch Table Cover", url: "/custom-print/table-cover/stretch-table-cover" },
					],
				},
			],
		},
	...
	const MenuList = ({ index, name, url, list, open }) => {
		const [openList, setOpenList] = React.useState(open);
		...
		return (
			<div onKeyDown={handleDrawer(false)}>
				<List key={index} style={{ position: "relative" }}>
					<ListItem button style={{ paddingTop: 2, paddingBottom: 2, paddingRight: 48 }} onClick={() => (list ? handleOpenList() : url ? handleRouterGo(url) : null)}>
						{url ? (
							<Link href={url}>
		...
		<React.Fragment>
			<AppBar position="fixed" className={clsx(classes.appBar, { [classes.appBarShift]: open })}>
				<Toolbar>
					...
					<Button classes={{ root: styles["root-button-title"] }} startIcon={<img src="/images/logo_dark.png" height={36} width={36} />} href="/" disableRipple>
			...
			<SwipeableDrawer className={classes.drawer} classes={{ paper: classes.drawerPaper }} anchor="left" open={open} onOpen={() => {}} onClose={handleDrawer(false)}>
				<div className={classes.drawerHeader}>
					<IconButton onClick={handleDrawer(false)}>{theme.direction === "ltr" ? <FontAwesomeIcon icon={faChevronLeft} color="white" /> : <FontAwesomeIcon icon={faChevronRight} color="white" />}</IconButton>
				</div>
