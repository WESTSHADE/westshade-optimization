...
import { FormControl, FormLabel, FormControlLabel, RadioGroup, Select, MenuItem } from "@material-ui/core";
...
export default function Selection_Group(props) {
	...
	if (type === "select") {
		...
		if (id === id_attribute_color || id === id_attribute_frameSeries) {
			return (
				...
				>
					...
					<RadioGroup row={direction === "row"} defaultValue={defaultValue} value={value} onChange={onChange}>
						{list.map((option, index) => (
							<FormControlLabel
								key={index}
								classes={{
		...
		return (
			<FormControl component="fieldset">
				{label ? (
					<FormLabel component="legend" classes={{ root: styles["root-formLabel"] }}>
						{label}
					</FormLabel>
				) : null}
				<RadioGroup row defaultValue={defaultValue} value={value} onChange={onChange}>
					{list.map((option, index) => (
						<FormControlLabel
							key={index}
							classes={{
